...
class RiakStream extends Readable {
   ...
   * @param {Object} options.client      a no-riak client instance.
   * @param {Object} [options.options]   per-request Riak options.
  ...
  #fetch(batch) {
    ...
    const requests = batch.map((current) =>
      Promise.resolve()
        ...
        .then((response) => {
          for (const data of [].concat(response ?? [])) {
   ...
   * @return {Promise<?(Object|Object[])>} the client response(s); `null` or
   *                                       `undefined` emits nothing (not found).
   */
  request(current) {
   ...
   * @abstract
   * @param  {Object} current map of dimension name to value.
   * @param  {Object} data    a single client response.
